...
Supports verified copy, proxies, local AI analysis, media stream, archive/restore batch, safe-delete (project-root only), and Resolve detect / scripting-probe / import-edl (V4: Media Pool bin + ImportMedia + EDL) / write-handoff / open (allowlisted only).
