...
export function inferFeatureFromPath(pathname: string): string {
  ...
  const rules: Array<[RegExp, string]> = [
    ...
    [/\/api\/script-writer\/sessions\/[^/]+\/feature\/(outline|act|assemble)$/, "script_writer.generate"],
    [/\/api\/script-writer\/sessions\/[^/]+\/storyboard\/generate$/, "script_writer.storyboard"],
    [/\/api\/script-writer\/idea-spark$/, "script_writer.ideas"],
    ...
    [/\/api\/revenue\/outreach\/ai-writer$/, "revenue.ai_writer"],
