...
export function W9UploadField({
  ...
  const staffUpload = !signingToken;
  const canUpload = signingToken || canUploadW9Docs(appUser);
  const canView = staffUpload && canViewW9Docs(appUser);
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-slate-50/80 p-4">
      ...
      {onFile && staffUpload && !canView && (
        <p className="mt-2 text-xs text-slate-500">
          W-9 is on file. Only accounting or admin can open the PDF.
        </p>
      )}
    </div>
  );
