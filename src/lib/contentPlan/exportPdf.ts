...
export function getContentPlanOnePagerFilename(plan: ContentPlan): string {
  const title = plan.creativeBrief?.workingTitle || plan.title || "content-plan";
  return `${sanitizeFilename(title)}-onset-one-pager.pdf`;
}
...
export function downloadContentPlanPdf(plan: ContentPlan): void {
  ...
  addLines(
    [
      ...
      plan.inputs.wardrobe ? `Wardrobe: ${plan.inputs.wardrobe}` : "",
...
/** Slim on-set one-pager: header, shoot order, pickups, checklist. */
export function downloadContentPlanOnePagerPdf(plan: ContentPlan): void {
  const doc = new jsPDF({ unit: "pt", format: "letter" });
  let y = MARGIN;
  ...
  setFont(9);
  doc.setTextColor(14, 165, 233);
  doc.text(`${APP_NAME} · On-set one-pager`, MARGIN, y);
  doc.setTextColor(15, 23, 42);
  y += 14;

  addLines(
    [
      `${plan.inputs.durationSeconds}s · ${plan.inputs.platform} · ${plan.inputs.orientation}`,
      plan.inputs.creatorName ? `Talent: ${plan.inputs.creatorName}` : "",
      plan.inputs.location ? `Location: ${plan.inputs.location}` : "",
      plan.inputs.wardrobe ? `Wardrobe: ${plan.inputs.wardrobe}` : "",
    ]
  ...
  const order =
    plan.shootOrderPlan?.shootOrder?.length
      ? plan.shootOrderPlan.shootOrder
      : (plan.shots || []).map((s) => ({
          shotId: s.id,
  ...
  if (order.length) {
    ...
    order.forEach((item, i) => {
      ...
      const lens = shot
        ? [shot.lens || shot.focalLength, shot.shotSize, shot.movement]
            .filter(Boolean)
            .join(" · ")
        : "";
      addBullet(
        `${i + 1}. S${String(item.shotNumber).padStart(2, "0")} ${item.shotName}${
          item.groupLabel ? ` [${item.groupLabel}]` : ""
        }${lens ? ` — ${lens}` : ""}`
      );
    });
  ...
  doc.save(getContentPlanOnePagerFilename(plan));
}
