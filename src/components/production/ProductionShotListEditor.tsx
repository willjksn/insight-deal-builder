    ...
    >
      <div className="mb-4 flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Coverage checklist — wide, medium, close-up per scene. Check off each shot as you capture
            it on set.
          </p>
        </div>
      ...
      {sorted.length === 0 ? (
        <p className="text-sm text-slate-500">
          No shots yet. Apply a script with detailed shot list, import from Shot Scout, or add shots
          manually.
        </p>
      ) : (
                ...
                >
                  ...
                  <td className="py-2 pr-2">
                    <div className="space-y-1.5">
                      <Input
                        value={shot.shotName ?? formatProductionShotLabel(shot).replace(/^\d+\.\s*/, "")}
                        disabled={readOnly}
                        onChange={(e) =>
                          update(shot.id, {
                      ...
                      {shot.notes && (
                        <p className="text-xs text-slate-400">{shot.notes}</p>
                      )}
                    </div>
                  </td>
                  <td className="py-2 pr-2">
