      ...
      <span className="text-sm">
        ...
        {!compact && (
          <span className="mt-0.5 block text-xs text-slate-500">
            ...
            inspiration images (ShotDeck, mood refs). Also generates full WS / MS / CU coverage
            for the shot list.
          </span>
        )}
        {compact && (
          <span className="ml-1 text-xs text-slate-500">
            — scene frames + full coverage
          </span>
        )}
      </span>
    </label>
  ...
  return (
    <div className={cn("space-y-2", className)}>
      <StoryboardModeToggle
        ...
        onChange={(on) => {
          onStoryboardChange(on);
          if (on) onDetailedChange(true);
        }}
        compact={compact}
      />
      {!storyboardMode && (
        ...
        >
          ...
          <span className="text-sm">
            ...
            {!compact && (
              <span className="mt-0.5 block text-xs text-slate-500">
                WS / MS / CU coverage per scene — checklist on the shot list page.
              </span>
            )}
          </span>
        </label>
